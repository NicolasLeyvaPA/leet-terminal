...
import (
	...
	"github.com/NicolasLeyvaPA/leet-terminal/internal/analyzer"
...
func main() {
	...
	logger.Info("Starting Analyzer Service", zap.String("environment", cfg.AppEnv))
	...
	engine := analyzer.NewEngine(db, redisClient, cfg, logger)

	// Create queue server for processing analysis jobs
	srv := queue.NewAsynqServer(cfg, logger)
	
	// Register task handlers
	mux := srv.Mux()
	mux.HandleFunc(queue.TypeAnalysisJob, engine.HandleAnalysisJob)
	...
	logger.Info("Analyzer service exited")
}
